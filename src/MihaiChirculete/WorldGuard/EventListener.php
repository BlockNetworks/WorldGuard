...
class EventListener implements Listener {
    ...
    const USABLES = [
        23, 25, 54, 58, 61, 62, 63, 64, 68, 69,
        70, 71, 72, 77, 84, 92, 93, 94, 96, 107,
        116, 117, 118, 125, 130, 131, 132, 137, 138, 143,
        145, 146, 147, 148, 149, 150, 154, 167, 183, 184,
    ...
    public function onInteract(PlayerInteractEvent $event)
    {
        ...
        if (isset($this->plugin->creating[$id = ($player = $event->getPlayer())->getRawUniqueId()])) {
            if ($event->getAction() === $event::RIGHT_CLICK_BLOCK) {
                ...
                $player->sendMessage(TextFormat::YELLOW . 'Selected position: X' . $x . ', Y: ' . $block->y . ', Z: ' . $z . ', Level: ' . $block->getLevel()->getName());
            ...
            if ($region->getFlag("pluginbypass") === "false") {
                if ($region->getFlag("block-place") === "false") {
                    if ($event->getPlayer()->hasPermission("worldguard.place." . $region->getName()) || $event->getPlayer()->hasPermission("worldguard.block-place." . $region->getName())) {
                        return true;
                    } elseif ($event->getPlayer()->hasPermission("worldguard.build-bypass")) {
                        return true;
                    } else {
    ...
    */
    ...
    {
        if ($event->getEntity()::NETWORK_ID !== 87) {
            return;
        }
